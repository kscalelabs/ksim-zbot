...
async def get_observation(kos: pykos.KOS) -> np.ndarray:
    ...
    # logger.info(f"vel_obs: {vel_obs}")
    imu_obs = np.array([imu.accel_x, imu.accel_y, imu.accel_z, imu.gyro_x, imu.gyro_y, imu.gyro_z])
    # logger.info(f"imu accel: {imu.accel_x}, {imu.accel_y}, {imu.accel_z}")
    # logger.info(f"imu gyro: {imu.gyro_x}, {imu.gyro_y}, {imu.gyro_z}")
